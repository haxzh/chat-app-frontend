...
//   // FIX: Prevent undefined in message
...
//           : "";
//       return msgPrefix + text.substring(0, 25);
//     }
//   };
...
//   // --- FIX: Unique users for key prop ---
//   function getData() {
//     if (searchKey === "") {
//       // Unique users from allChats
//       const users = [];
...
//             users.push({ user, chatId: chat._id });
...
//         )
//         .map((user) => ({ user, chatId: user._id })); // fallback chatId for key
//     }
//   }

//   return getData().map(({ user, chatId }) => (
//     <div
...
function UsersList({ searchKey, socket, onlineUser }) {
  ...
  // SLIDE/SCROLLABLE: Wrap user list in a scrollable div
  return (
    <div style={{ height: "500px", overflowY: "auto" }}>
      {getData().map(({ user, chatId }) => (
        <div
          className="user-search-filter"
          onClick={() => openChat(user._id)}
          key={chatId}
